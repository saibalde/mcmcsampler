...
TEST(McmcSampler, DelayedRejection) {
  ...
  Eigen::MatrixXd proposal_covariance(num_dim, num_dim);
  proposal_covariance << 100.0 / 19.0, -90.0 / 19.0, -90.0 / 19.0, 100.0 / 19.0;
  Eigen::VectorXd initial_sample(num_dim);
  initial_sample << 0.0, -1.0;
  unsigned long num_sample = 100000;
  double delayed_rejection_scaling = 0.5;
  std::mt19937 rng(0);

  Eigen::MatrixXd samples = mcmcsampler::DR(
      num_dim, negative_log_pdf, proposal_covariance, initial_sample,
      num_sample, delayed_rejection_scaling, rng);

  WriteMatrix(samples, "dr_test.dat");
}
