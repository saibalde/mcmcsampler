...
TEST(McmcSampler, DelayedRejectionAdaptiveMetropolis) {
  ...
  Eigen::MatrixXd samples = mcmcsampler::DRAM(
      num_dim, negative_log_pdf, initial_covariance, initial_sample, num_sample,
      adaptive_metropolis_threshold, adaptive_metropolis_regularization,
      delayed_rejection_scaling, rng);

  WriteMatrix(samples, "dram_test.dat");
}
